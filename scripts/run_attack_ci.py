...
def run(config: str, timeout_s: int, out_json: Path) -> int:
    ...
    controller = PACController(
        ...
        leakage_budget=18.0,
        leakage_rate=0.02,
        noise_std=0.01 if protected else 0.0,
        clip_min=1e-5,
        clip_max=1.0,
    )
    ...
    try:
        pac_bound = controller.pac_bound(n_queries=len(x_query))
    except Exception as exc:  # noqa: BLE001 - explicit CI failure path
